...
BEGIN
    ...
    -- Check 'email' length and uniqueness
    ...
    -- Check 'role_id' existence in 'roles' table
...
BEGIN
    ...
    -- Check if 'post_id' references an existing post in 'posts' table
    IF NOT EXISTS (SELECT 1 FROM posts WHERE id = NEW.post_id) THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Invalid post_id. Post does not exist.';
    END IF;

    -- Check if 'user_id' references an existing user in 'users' table
...
FOR EACH ROW
BEGIN
    -- Check if the combination of 'post_id' and 'user_id' is unique
